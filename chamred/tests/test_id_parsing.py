...
def test_multiple_id_parsing():
    multiple_ids_file = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "test_data", "card_ids.txt"
    )
    assert [
        "ARO:3001816",
        "ARO:3002012",
...
def test_harmonizarion_parsing(expected_harmonizarion_parsing_output):
    hamronization_json_file = os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "test_data", "ncbi_summary.json"
    )
    assert (
        expected_harmonizarion_parsing_output
        == utility_functions.parse_hamronization_json_file(hamronization_json_file)
    )
