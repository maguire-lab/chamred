        ...
        for node_id, node_data in graph.nodes(data=True):
            if (
                ...
                or (
                    ...
                    and identifier
                    in itertools.chain(*node_data["duplicate_allele_ids"].values())
                )
                or node_data["name"] == identifier
...
def __add_connection(
    ...
    if graph.has_edge(source_composite_key, target_composite_key):
        if debug:
            print(
                f"Warning! Graph already has edge {source_composite_key} => {target_composite_key}"
            )
    else:
    ...
    if edge_attributes["type"] == "RBH":
        if graph.has_edge(target_composite_key, source_composite_key):
            if debug:
                print(
                    f"Warning! Graph already has edge {target_composite_key} => {source_composite_key}"
                )
        else:
            graph.add_edge(
                target_composite_key, source_composite_key, **edge_attributes
            )
