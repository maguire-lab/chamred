...
def single_node_info_text(
    identifier, database, graph, coverage_threshold=0.9, identity_threshold=0.9
):
    """
    ...
    source_node_id, source_node_data = graph_functions.get_graph_node(
        identifier, database, graph
    )
    ...
    for database in node_targets:
        ...
        if (
            edge_data["coverage"] < coverage_threshold
            or edge_data["identity"] < identity_threshold
        ):
        ...
        other_data = {
            key: target_node_data[key]
            for key in target_node_data
...
def write_multiple_node_info(
    id_data, graph, out_filepath, coverage_threshold=0.9, identity_threshold=0.9
):
    """
    ...
    """
    all_databases = set(
        [graph.nodes[node_name]["database"] for node_name in graph.nodes]
    )
    ...
    with open(out_filepath, "w") as out:
        header = __multiple_ids_header(
            header_databases, target_node_field_titles, multiple_samples
        )
        out.write(f"{header}")
        missing_ids = {}
        for id_info in track(id_data, description=f"Finding metadata"):
            ...
            source_node_id, source_node = graph_functions.get_graph_node(
                source_id, source_database, graph
            )
            if not source_node_id:
            ...
            metadata_string = ", ".join(
                [
                    f"{key}: {value}"
                    for key, value in __node_metadata(source_node).items()
                ]
            )
            ...
            for target_database in source_node_target_info:
                best_match = __best_target_node_match(
                    source_node_target_info[target_database],
                    coverage_threshold,
                    identity_threshold,
                )
                if best_match:
                    target_node_info[target_database] = {
                        ...
                        "metadata": ", ".join(
                            [
                                f"{key}: {value}"
                                for key, value in __node_metadata(
                                    best_match["node"]
                                ).items()
                            ]
                        ),
            ...
            for target_database in header_databases:
                for field in target_node_field_titles:
                    if (
                        ...
                        or target_database not in target_node_info
                    ):
                        target_node_info_list.append("-")
                    else:
                        target_node_info_list.append(
                            str(target_node_info[target_database][field])
                        )
            target_node_info_string = "\t".join(target_node_info_list)
            if multiple_samples:
                source_file = id_info["file"]
                out.write(
                    f"{source_file}\t{source_id}\t{source_database}\t{name}\t{metadata_string}\t{target_node_info_string}\n"
                )
            else:
                out.write(
                    f"{source_id}\t{source_database}\t{name}\t{metadata_string}\t{target_node_info_string}\n"
                )
...
def __node_target_info(graph, node_id):
    ...
    for edge in graph.edges(node_id):
        ...
        node_targets[database][id] = {"node": target_node, "edge_data": edge_data}
    return node_targets
    ...
    if len(missing_ids) > 0:
        ...
        for database in missing_ids:
            ...
            for missing_id in missing_ids[database]:
                ...
                print(
                    f"\t{missing_id}{database_name_string} in {num_missing_ids}{proportion} samples"
                )
...
def __get_name_for_id(id_to_match, metadata):
    ...
    for database_id in metadata:
        if id_to_match == database_id:
            ...
            and id_to_match
            in itertools.chain(*metadata[database_id]["duplicate_allele_ids"].values())
        ):
            return metadata[database_id]["name"]
    return None
