...
def parse_arguments():
    ...
    query = subparsers.add_parser(
        "query", help="Query databases for matches and associated metadata"
    )
    query.add_argument(
        "-d",
        "--database",
        help="which database are the gene(s) in",
        choices=["card", "ncbi", "resfinder"],
        required="-i" in sys.argv
        or "--id" in sys.argv
        or "-f" in sys.argv
        or "--id_file" in sys.argv,
    )
    query.add_argument(
        "-ct",
        "--coverage_threshold",
        help="coverage threshold below which a match will not be reported",
        type=float,
        default=0.9,
    )
    query.add_argument(
        "-it",
        "--identity_threshold",
        help="identity threshold below which a match will not be reported",
        type=float,
        default=0.9,
    )
    id_source = query.add_mutually_exclusive_group(required=True)
    id_source.add_argument(
        "-i", "--id", help="The id of a ARG in the specified database"
    )
    id_source.add_argument(
        "-f",
        "--id_file",
        help="Path to a file containing ids of ARGs in the specified database",
        type=lambda x: is_valid_file(parser, x),
    )
    id_source.add_argument(
        "-j",
        "--hamronization_json_file",
        help="Path to a hamronization summaty in JSON format",
        type=lambda x: is_valid_file(parser, x),
    )
    query.add_argument(
        "-o",
        "--outfile_path",
        help="Path to file where query results will be written",
        required="-f" in sys.argv
        or "--id_file" in sys.argv
        or "-j" in sys.argv
        or "--hamronization_json_file" in sys.argv,
    )
